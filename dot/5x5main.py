...
def main():
    level = input("input a difficulty s/m/h:")
    if level != "s" and level!= "m" and level != "h":
        print("input wrong level auto set to simple")
        level = "s"
    pygame.init()
    white = (255, 246, 143) 
...
def high_player(game):
    ...
    for i in range(0,game.r):
        for j in range(0,game.c):
            if i%2!=0 and j%2!=0 and game.board[i,j] == 0:
                if game.check_empty_lines(i,j) is not False:
                    return random.choice(game.check_empty_lines(i,j))
    for i in range(0,game.r):
        for j in range(0,game.c):
            if i%2!=0 and j%2!=0 and game.board[i,j] == 0:
                if game.check_two_lines(i,j):
